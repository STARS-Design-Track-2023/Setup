...
# Cell libraries
GATE_LIB         := AMI_05
CELLS            := sky130_fd_sc_hd__tt_100C_1v80
...
$(MAP): $(addprefix $(SRC)/, $(TOP_FILE) $(COMPONENT_FILES) $(TB))
	@echo "----------------------------------------------------------------"
	@echo "Synthesizing and Compiling with  process ....."
	@echo "----------------------------------------------------------------\n\n"
	@mkdir -p ./$(MAP)
	@mkdir -p ./$(BUILD)
	@touch -c $(TOP).log
	...
	@$(VC) $(CFLAGS) -o $(BUILD)/$(SIM_MAPPED).vvp -DFUNCTIONAL -DUNIT_DELAY=#1 $@/$(TOP_MODULE).v $(SRC)/$(TB) $(GATE_LIB)/primitives.v $(GATE_LIB)/sky130_fd_sc_hd.v
	@echo "\n\n"
	@echo "Compilation complete\n\n"
